      ...
      >
        ...
        <div className="flex items-start justify-between p-5 border-b border-border-default">
          <div className="min-w-0 pr-4">
            ...
            <p className="text-xs text-foreground-muted mt-0.5">{p.sku}</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
              ...
              <div>
                ...
                <p className="text-foreground font-medium">{p.gst_rate != null ? `${p.gst_rate}%` : '—'}</p>
              </div>
              <div>
                <p className="text-xs text-foreground-muted">Cost Price</p>
                <p className="text-foreground font-medium">
                  {p.cost_price ? `Rs. ${Number(p.cost_price).toLocaleString('en-IN', { minimumFractionDigits: 2 })}` : '—'}
                </p>
              </div>
            </div>
        ...
        {/* Variants table */}
        {loading && (
          <div className="px-5 pb-4 text-sm text-foreground-muted">Loading variants…</div>
        )}
