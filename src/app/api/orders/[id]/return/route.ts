...
export async function POST(
  ...
  try {
    ...
    const order = await queryOne(
      `SELECT o.id, o.status, o.order_number, o.delivered_at, o.user_id,
    ...
    if (!order.delivered_at) {
      return NextResponse.json({ error: 'Delivery date not recorded. Please contact support.' }, { status: 400 })
    }
